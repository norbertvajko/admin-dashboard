...
export default function CookieConsent({
  ...
  // Fetch and add location info only after user accepts
  const accept = async () => {
    setIsOpen(false);
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Get the existing info (device + visitedDate)
        const existingInfo = JSON.parse(localStorage.getItem("userInfo") || "{}");

        // Add location data without overwriting device or visitedDate
        const updatedInfo = {
          ...existingInfo,
          location: {
            data
          ...
          acceptedDate: new Date().toISOString(), // track when consent was accepted
        };
